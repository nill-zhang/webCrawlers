...
class MarathonSpider(Spider):
    ...
    def parse(self, response):
        year_urls = response.xpath("//b[contains(text(),'Marathon Results') and "
                                   "string-length(text()) >20]/following-sibling::a/@href")\
                                    .extract()
    ...
    def parse_one_section(self, response):
        ...
        for entry in raw_entries:
            yield dict(enumerate(entry.xpath("td/text()").extract()))
